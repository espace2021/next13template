...
function Preview({ product }) {
    ...
    return <div className='preview'>
        ...
        <Image className='img' src={`${product.imageart}`} alt={product.designation} width={512} height={512} />
...
function Details({ product }) {
    ...
    return <div className='details'>
        ...
        <h1 className='name' >{product.designation}</h1>

        <div className='price-box'>
            <span className='price'>{'$' + (product.prix / 100).toFixed(2)}</span>
            <span className='price_compare' >{'$' + (product.prix / 100).toFixed(2)}</span>
        </div>

        {process.env.NEXT_PUBLIC_REVIEWS === 'true' && <div className='stars-box'>
            <Stars value={2} total={4} />
        </div>}
